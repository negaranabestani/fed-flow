...
sys.path.append('../')
from app.RL_training.RLEnv import Env
import app.config.config as config
import app.util.rl_utils as utils
import app.RL_training.RLEnv
import app.model.entity.rl_model.PPO as PPO
...
vggModel = vgg()
env = Env(0, config.SERVER_ADDR, config.SERVER_PORT, config.CLIENTS_LIST, config.model_name, vggModel.get_config(),
          config.rl_b)
...
env = utils.createEnv(iotDeviceNum=config.K,
                      edgeDeviceNum=config.S,
                      timestepNum=config.max_timesteps,
                      fraction=0.8)

saveAgentPath = f"{Path(__file__).parent}"
print(saveAgentPath)
